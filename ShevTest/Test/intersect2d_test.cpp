...
//    for ( nat j = 0; j < 15; ++j )
    ...
    /*/
    {
        DynArray< DynArray<Vector2d> > plus, minus;
        cutPolygon ( vert, line, plus, minus );
        for ( nat i = 0; i < minus.size(); ++i )
        {
            drawPolygon ( minus[i], 1, 0, 1 );
        }
        for ( nat i = 0; i < plus.size(); ++i )
        {
            drawPolygon ( plus[i], 0, 1, 0 );
        }
    }
    //*/
}
