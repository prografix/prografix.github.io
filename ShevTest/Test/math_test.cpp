...
// Запись номеров строк в массив столбцов
 ...
 // Запись строк в очередь
    ...
    for ( k = 0; k < n; ++k )
    {
        ...
        CCArrRef<SortItem<nat, double> > & row = a[k];
        nat im = 0;
...
struct MatrElem
{
    nat row; 
    double value;
    TreeElem * prevCol, * nextCol;
    TreeElem * prevRow, * nextRow;
};
...
struct SparseMatrix
{
    ...
    void delCol ( nat i )
    {
        ...
        while ( t )
        {
            ...
            t = e->nextRow;
            if ( e->nextCol )
                e->nextCol->data.prevCol = e->prevCol;
            if ( e->prevCol )
                e->prevCol->data.nextCol = e->nextCol;
            else
                row[e->row] = e->nextCol;
        
            if ( e->nextRow )
                e->nextRow->data.prevRow = e->prevRow;
            if ( e->prevRow )
...
// Поиск максимального по модулю элемента в строке
        ...
        row2[k] = em;
// Нормализация строки
        const double p = 1. / em->data.value;
        e = rk;
        while ( e )
        {
            MatrElem & me = e->data;
...
// Вычитание строк
        ...
        while ( c )
        {
            ...
            while ( e )
            {
                ...
                if ( ej )
                {
                   // if ( e != em ) // если элемент ej не находится на ведущем столбце
...
const char * test ( const SparseMatrix2L & m )
{
    const char * err = 0;
    for ( nat i = 0; i < m.n; ++i )
    {
        MatrElem2L * e = m.row[i];
        nat rc = 0;
        while ( e )
        {
            ++rc;
    ...
    for ( nat i = 0; i < m.n; ++i )
    {
        MatrElem2L * e = m.col[i];
        while ( e )
        {
            if ( e->col != i )
            { err = "t->col != i"; goto m1; }
            if ( e == m.col[i] && e->prevRow )
            { err = "f->prevRow != 0"; goto m1; }
...
struct SimpleMatrElemStor : public IMatrElem2LStor
{
    virtual MatrElem2L * get() { return new MatrElem2L; }
    virtual void put ( MatrElem2L * p ) { delete p; }
};

struct MatrElem2LStor : public IMatrElem2LStor
{
    MatrElem2L * ptr;
    MatrElem2LStor() : ptr(0) {}
   ~MatrElem2LStor()
    {
        while ( ptr )
        {
            MatrElem2L * p = ptr->nextCol;
            delete ptr;
            ptr = p;
        }
    }
    virtual MatrElem2L * get()
    {
        if ( ptr )
        {
            MatrElem2L * p = ptr;
            ...
            return new MatrElem2L ;
    }
    virtual void put ( MatrElem2L * p )
    { 
        p->nextCol= ptr;
        ptr = p;
    }
...
void sluGaussRowT_test1()
{
    static PNormalRand rand;
//    static PRand rand;
    const nat n = 2000, m = 27;
    double c[n], x[n], b[n];
    for ( nat i = 0; i < n; ++i ) c[i] = rand();
    MatrElem2LStor stor;
    SparseMatrix2L matr ( n, stor );
    nat k = 0;
    for ( nat i = 0; i < n; ++i )
    {
        b[i] = 0;
...
//display << x[i] << c[i] << NL;
        ...
        display << double(n*n)/k << max << t1-t0 << NL;
    }
    else
        display << "no" << NL;
}
...
//    static PRand rand;
    //AVL_TreeNodeStor<nat, MatrElem> stor;
    MatrElem2LStor stor;
    for ( nat n = 10; n < 1000; ++n )
    {
    const nat m = 6;
    DynArray<double> c(n), x(n), b(n);
    for ( nat i = 0; i < n; ++i ) c[i] = rand();
    SparseMatrix2L matr ( n, stor );
    //SparseMatrix matr ( n, stor );
...
//display << x[i] << c[i] << NL;
        ...
        display << double(n*n)/k << max << t1-t0 << n << NL;
    }
    else
        display << "no" << NL;
    }
...
//    t.func();
    ...
    {
        a[i].resize(5);
        for ( nat j = 0; j < 5; ++j ) a[i][j].head = j;
    }
    a[0][0].tail =    36; a[0][1].tail =    -630; a[0][2].tail =    3360; a[0][3].tail =    -7560; a[0][4].tail =    7560;
    a[1][0].tail =  -630; a[1][1].tail =   14700; a[1][2].tail =  -88200; a[1][3].tail =   211680; a[1][4].tail =  -220500;
    a[2][0].tail =  3360; a[2][1].tail =  -88200; a[2][2].tail =  564480; a[2][3].tail = -1411200; a[2][4].tail =  1512000;
    a[3][0].tail = -7560; a[3][1].tail =  211680; a[3][2].tail =-1411200; a[3][3].tail =  3628800; a[3][4].tail = -3969000;
    a[4][0].tail =  7560; a[4][1].tail = -220500; a[4][2].tail = 1512000; a[4][3].tail = -3969000; a[4][4].tail =  4410000;
    double x[5], b[5];
    b[0] = 463; b[1] = -13860; b[2] = 97020; b[3] = -258720; b[4] = 291060;
    sluGaussRowO ( 5, a, b, x );
    for ( nat i = 0; i < 5; ++i ) display << x[i] << c[i] << NL;
}
...
void chol2 ( nat n, Suite<SortItem<nat, double>> * a, double * b, double * x )
{
    if ( ! n ) return;
    nat i, j, k;
    for ( k = 0; k < n; ++k )
    ...
    */
        for ( j = 0; j < ak.size(); ++j )
        {
            ...
            {
                SortItem<nat, double> & si = ak[i];
                const double p = - si.tail * sj.tail;
                for (;;)
                {
                    if ( l == aj.size() )
                    {
                        aj.inc() = SortItem<nat, double> ( si.head, p );
                        break;
                    }
...
// Решение системы Ly = b
    ...
    for ( i = 0; i < n; ++i )
    {
        ...
        for ( k = 0; k < ai.size(); ++k )
        {
            const SortItem<nat, double> & si = ai[k];
            if ( si.head < i ) continue;
            if ( si.head == i )
            {
                x[i] = b[i] * si.tail;
            }
            ...
            {
                b[si.head] -= si.tail * x[i];
            }
        }
    }
    /*for ( i = 0; i < n; ++i )
    {
        CCArrRef<SortItem<nat, double>> & ri = a[i];
        double t = b[i];
        for ( k = 0; k < i; ++k )
        {
            t -= g[p] * x[k];
