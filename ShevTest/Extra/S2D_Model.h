...
public:
    ...
    S2D_Model & cut2 ( const Double<N+1> & g, S2D_ModelStor<N> & stor );
};
...
S2D_Model<N> & S2D_Model<N>::cut ( const Double<N+1> & plane, S2D_ModelStor<N> & stor )
{
    ...
    do
    {
        verts.cur()->temp = plane % verts.cur()->base();
    }
    while ( verts.next () );
...
if ( varr.size() > 2 )
    s = s;
        if ( varr.size() > 0 )
        {
            ...
            e->plane[N-2] = key[N-3] = planes.las();
            e->side[N-2] = s;
            for ( i = 0; i < N-2; ++i )
            {
                nat k = 0;
...
double t1 = timeInSec();
                ...
                do
                {
                    S2D_Side<N> * p = stemp.cur();
                    for ( nat j = 0; j < N-3; ++j )
                    {
++scount;
                        if ( key[j] != p->plane[j] ) goto m1;
                    }
...
m1:;            }
                while ( stemp.next() );
                {
                    S2D_Side<N> * p = stemp1.addNewAftLas ( stor.sides );
                    for ( nat j = 0; j < N-2; ++j ) p->plane[j] = key[j];
                    p->addAftLas ( & e->edgePtr[i] );
                    e->side[i] = p;
                }
m2:;
...
template <nat N> 
S2D_Model<N> & S2D_Model<N>::cut2 ( const Double<N+1> & plane, S2D_ModelStor<N> & stor )
{
    if ( ! verts.size() || ! edges.size() || ! sides.size() ) return *this;
    verts.top();
    do
...
double t1 = timeInSec();
                ...
                {
                    S2D_Side<N> * p = stemp.cur();
                    const Kit<S2D_Plane<N> *, N-3> & r2 = *(Kit<S2D_Plane<N> *, N-3>*) p->plane;
                    if ( key == r2 )
                    {
                        p->addAftLas ( & e->edgePtr[i] );
                        e->side[i] = p;
